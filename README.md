...
- PUT (Adresse aktualisieren): 
  ```
  PUT /api/addresses/<url-encodierter Schlüssel>
  ```
  mit komplettem JSON-Objekt (wie bei POST)
- DELETE: 
  ```
  DELETE /api/addresses/<url-encodierter Schlüssel>
  ```
...
- Fehler beim Hinzufügen: "Invalid JSON from server" — sicherstellen, dass der Server immer gültiges JSON zurückgibt.
